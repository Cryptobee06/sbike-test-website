...
function MarkdownContent({ content }) {
  ...
  const html = content
    ...
    .replace(/(\n<li[^>]*>.*?<\/li>\n?)+/gs, '<ul class="list-disc list-inside mb-4 space-y-1">$&</ul>')
    .replace(/\n\n/g, '</p><p class="text-gray-700 leading-relaxed mb-4">')
    .replace(/^/, '<p class="text-gray-700 leading-relaxed mb-4">')
    .replace(/$/, '</p>');

  return <div className="prose max-w-none box-white-border" dangerouslySetInnerHTML={{ __html: html }} />;
}
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center">
            <a href={`/${locale}/blog`} className="inline-flex items-center px-6 py-3 bg-primary-600 text-white rounded-lg font-semibold hover:bg-primary-700 transition-colors">
